...
export const analyzeSolution = onRequest(async (req, res) => {
    cors(req, res, async () => {
        ...
        const playerResponse = req.body.player_response;
        const playerName = req.body.player_name;

        if (!scenarioId) {
            res.status(403).json({error: "Scenario ID is required"})
            return
        }

        if (!playerResponse) {
            res.status(403).json({error: "Player response is required"})
            return
        }

        if (!playerName) {
            res.status(403).json({error: "Player name is required"})
            return
        }
        ...
        if (error || !scenario) {
            res.status(403).json({error: "Scenario not found"})
            return
        }
...
1. Carefully analyze the player's response (${playerResponse}) and compare it with the solution hints (${scenario.solutions}).
...
Return the outcome in **pure JSON format (not in markdown \`\`\`json\`\`\`)** with the following structure, and **ensure that the generated text is in French**:
{
  ...
  "result": "success" or "failure"
    ...
    `;
        ...
        res.json(JSON.parse(iaResponse.choices[0].message.content ?? "{}"))
    })
